...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // создадим сервер, который отвечает на нужные запросы, слушает порт 8989
        // отвечать на запросы /{word} -> возвращённое значение метода search(word) в JSON-формате
        try (ServerSocket serverSocket = new ServerSocket(SERVER_PORT);) {
            System.out.println("Сервер работает на порту " + SERVER_PORT);
            while (true) {
    ...
    // Обработка запроса, формирование json-текста
